...
public class FBXCompressor {
    ...
    private static void compressFile(Scanner scanner) {
        try {
            ...
            if (!FilenameUtils.getExtension(inputPath).equalsIgnoreCase("fbx")) {
                System.err.println("Error: Input file must be an FBX file (.fbx extension).");
                return;
            }
            ...
            String outputFileName = FilenameUtils.getBaseName(inputFile.getName()) + "_compressed.fbx";
            ...
            CompressionResult result = compressFBX(inputFile, outputPath.toFile());
    ...
    private static void decompressFile(Scanner scanner) {
        try {
            ...
            if (originalName.endsWith("_compressed.fbx")) {
                originalName = originalName.replace("_compressed.fbx", ".fbx");
            } else if (originalName.endsWith(".fbx")) {
                originalName = originalName.replace(".fbx", "_decompressed.fbx");
            } else {
                originalName += ".fbx";
            }
    ...
    private static CompressionResult compressFBX(File inputFile, File outputFile) throws IOException {
        // Only check extension, not content, to be more permissive
        String fileName = inputFile.getName().toLowerCase();
        if (!fileName.endsWith(".fbx")) {
            throw new IOException("Input file must have .fbx extension");
        }
